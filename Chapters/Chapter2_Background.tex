...
Similar to the action languages, $\mathcal{APL}$ requires that the agent's environment be modeled by a propositional transition system $T=<\{t,f\},\boldsymbol{F},\boldsymbol{A}>$.
...
\begin{definition}
    An authorization policy $P$ for $T$ is \textit{consistent} if for every state $s$ of $T$, logic program $lp(P, o)$ is consistent.
\end{definition}

\begin{definition}
    An authorization policy $P$ for $T$ is \textit{categorical} if for every state $s$ of $T$, logic program $lp(P, s)$ is categorical.
\end{definition}
...
The logic translation of $P_m$, $lp(P_m)$ is as follows:

\begin{lstlisting}[language=Prolog]
-permitted(assume_command(c_1, m_1)) :-
    authorized(c_1, m_1),
    not ab(d_1),
    not permitted(assume_command(c_1, m_1)).
...
To check the compliance of action $a = assume_command(c_1, m_1)$, one would first check to see if the event $<s_0, a>$ is strongly compliant.

\subsubsection{Checking compliance with complete knowledge of state}

\begin{definition}
    An event $<s, a>$ is \textit{strongly compliant} with a consistent policy $P$ for $T$ iff the logic program $lp(P, s) \cup \{ \leftarrow permitted(a) \}$ is inconsistent.
\end{definition}

\begin{definition}
    An event $<s, a>$ is \textit{weakly compliant} with a consistent policy $P$ for $T$ iff the logic program $lp(P, s) \cup \{ \leftarrow \neg permitted(a) \}$ is consistent.
\end{definition}

\begin{definition}
    An event $<s, a>$ is \textit{non-compliant} with a consistent policy $P$ for $T$ iff the logic program $lp(P, s) \cup \{ \leftarrow \neg permitted(a) \}$ is inconsistent.
\end{definition}

\subsubsection{Checking compliance with partial knowledge of state}

\begin{definition}
    For all actions $a \in \delta(s)$, the event $<s, a>$ is \textit{strongly compliant} with $P$ iff the program $lp(P, s) \cup D \cup lp(SL) \cup \{ \leftarrow permitted(a) \}$ is inconsistent.
\end{definition}

\begin{definition}
    For all actions $a \in \delta(s)$, the event $<s, a>$ is \textit{weakly compliant} with $P$ if $P$ is categorical and the program $lp(P, s) \cup D \cup lp(SL) \cup \{ \leftarrow \textbf{ not } \neg permitted(a) \}$ is inconsistent.
\end{definition}

\begin{definition}
    For all actions $a \in \delta(s)$, the event $<s, a>$ is \textit{non-compliant} with $P$ iff the program $lp(P, s) \cup D \cup lp(SL) \cup \{ \leftarrow \neg permitted(a) \}$ is inconsistent.
\end{definition}

\begin{definition}
    If $P$ is categorical, then an event $<s, a>$, where $s \in \delta(s)$, is also non-compliant with $P$ if the program $lp(P,s) \cup D \cup lp(SL) \cup \{ \leftarrow \textbf{ not } \neg permitted(a) \}$
\end{definition}

\subsubsection{Checking compliance with minimal knowledge of state}

\begin{definition}
    A model of history $H_n$ is \textit{strongly compliant} with $P$ iff the program $lp(P, H_n) \cup D \cup lp(A) \cup Check_1$ is inconsistent.
\end{definition}

\begin{definition}
    A model of history $H_n$ is \textit{weakly compliant} with $P$ iff the program $lp(P, H_n) \cup D \cup lp(A) \cup Check_2$ is inconsistent.
\end{definition}
...
\begin{definition}
    Let $P_a$ be a derivative policy of $P$ that only has the authorization rules of $P$.
    $P_a$ is an authorization policy \textit{induced} by $P$.
\end{definition}

\begin{definition}
    Likewise, let $P_o$ be a derivative of $P$ such that $P_o$ only has the obligation rules of $P$.
