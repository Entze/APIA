...
For example, a controller may require an agent to follow ethical constraints and may penalize an agent when acting in violation of them.
Thus, it is necessary to introduce a discussion on agent policies.

\citet{gelfond_authorization_2008} introduce language APL to represent authorization policies.
...
APL is not an action language in and of itself since it only describes whether actions are permitted.
APL relies on languages such as A, B, and AL to describe the effects of actions.

Similar to the action languages, APL requires that the agent’s environment be modelled by a transition diagram.
\citet{gelfond_authorization_2008} use the transition diagram definition extended with sets of simultaneous actions.
