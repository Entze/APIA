...
Many action languages exist, including A \cite{gelfond_action_1998}, B \cite{gelfond_action_1998}, C \cite{gelfond_action_1998}, AC \cite{turner_representing_1997}, AL \cite{baral_reasoning_2000}, ALM \cite{inclezan_modular_2016}.

\subsubsection{Action language A}

First, we will discuss action language $ A $.
Action language $ A $ was first proposed in \cite{pednault_formulating_1987}.
...
\begin{definition}{Action description}
    Let $ D $ be an action description in $ A $.
...
\subsubsection{Action language B}

Action language B is an extension of action language A \cite{gelfond_action_1998} that allows the description of actions with indirect effects.

\subsubsection{Action language AL}

AL is based on A and B.
