...
\begin{definition}
    ...
    \begin{itemize}
        \item A \textit{permission} is a statement of the form $permitted(a)$.
...
\begin{definition}
    ...
    \begin{itemize}
        \item $permitted(a) \in P(s)$ iff a logic program $lp(P, s)$ entails $permitted(a)$
...
\lstinputlisting{Figures/APL_Language/Example_1/clingo.txt}
...
Let $D = \{ (f,y) \in \boldsymbol{F} \times \boldsymbol{V} | lp(V(f,s)=y)\}$
...
\noindent
where $F\subseteq\left(\{(f, y) \in \boldsymbol{F} \times \boldsymbol{V} | V(f,s)=y\}\right) \cup (\{obl(a), \\
\neg obl(a), \\
\textbf{normally } obl(a), \\
\textbf{normally } \neg obl(a)\}$ and $a \in \boldsymbol{A} \cup \{a' \in \boldsymbol{A} | \neg a'\}$.
