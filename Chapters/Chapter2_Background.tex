...
\begin{definition}
    ...
    where
    \[
        F\subseteq\left(\boldsymbol{F}\cup\{f\in \boldsymbol{F}|\neg f\}\right) \cup (\{permitted(<s,A>), \\
        \neg permitted(<s, A>), \\
...
\begin{definition}
    ...
    \begin{itemize}
        \item An authorization policy $P$ for $T$ is \textit{consistent} if for every state $s$ of $T$, logic program $lp(P, o)$ is consistent.
...
\begin{lstlisting}
$ clingo Pm.lp s0.lp --warn no-atom-undefined
...
Considering actions $A=\{assume\_command\}$, since $a = assume\_command(c_1, m_1)$ is the only element of $A$ and $lp(P_m, s_0)$ entails $permitted(assume\_command(c_1, m_1))$, $A$ is stronly compliant with $P_m$.
...
\begin{definition}
    ...
    \begin{itemize}
        \item For all actions $a \in \delta(s)$, the event $<s, a>$ is \textit{strongly compliant} with $P$ iff the program $lp(P, s) \cup D \cup lp(SL) \cup \{ \leftarrow permitted(a) \}$ is inconsistent.
        \item For all actions $a \in \delta(s)$, the event $<s, a>$ is \textit{weakly compliant} with $P$ if $P$ is categorical and the program $lp(P, s) \cup D \cup lp(SL) \cup \{ \leftarrow \textbf{ not } \neg permitted(a) \}$ is inconsistent.
        \item For all actions $a \in \delta(s)$, the event $<s, a>$ is \textit{non-compliant} with $P$ iff the program $lp(P, s) \cup D \cup lp(SL) \cup \{ \leftarrow \neg permitted(a) \}$ is inconsistent.
        \item If $P$ is categorical, then an event $<s, a>$, where $s \in \delta(s)$, is also non-compliant with $P$ if the program $lp(P,s) \cup D \cup lp(SL) \cup \{ \leftarrow \textbf{ not } \neg permitted(a) \}$
    \end{itemize}
\end{definition}
