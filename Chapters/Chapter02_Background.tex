...
\begin{gather}
    ...
    H_1 = < s_0, A_0 >
\end{gather}
and we want to check the compliance of action $A_1 = \{ assume\_command(c_1, m_1) \}$.
...
\begin{definition}
    ...
    \begin{gather}
        ...
        prefer(d_i, d_j)
    \end{gather}
    where $s \in S$, $h \in \boldsymbol{A} \cup \{\neg a | a \in \boldsymbol{A}\}$, and
    \begin{multline}
        F\subseteq\left(\{V(f,s)=y | (f, y) \in \boldsymbol{F} \times \boldsymbol{V}\}\right) \cup (\{obl(h),
            \neg obl(h), \\
            \textbf{normally } obl(h),
            \textbf{normally } \neg obl(h)\}
    \end{multline}
\end{definition}
...
\end{gather}
\begin{multline}
    lp(d: \textbf{normally } obl(h) \textbf{ if } F) =_{def}
        obl(h) \leftarrow \\
            lp(F),
            \textbf{not } ab(d),
            \textbf{not } \neg obl(h). \\
