...
\footnotetext{
    \textsc{clingo}~\citep{gebser_potassco_2011} is available for download at this URL: \url{https://potassco.org/clingo/}
}
...
\footnotetext{
    ...
    The provided definition a simple logic negation of this definition and, hence, is equivalent to it.
}
...
$D_0$ is as follows:
\lstinputlisting[language=Prolog]{Figures/APL_Language/04_Minimal_Knowledge/D0.lp}
...
\lstinputlisting{Figures/APL_Language/04_Minimal_Knowledge/Check_1.clingo.txt}

The output of logic program \cref{eq:authorization_history_knowledge_weakly_compliant} is as follows:
\lstinputlisting{Figures/APL_Language/04_Minimal_Knowledge/Check_2.clingo.txt}
...
\lstinputlisting[language=Prolog]{Figures/APL_Language/04_Minimal_Knowledge/Inertia_Axiom.lp}
...
\lstinputlisting{Figures/APL_Language/04_Minimal_Knowledge/Check_1_with_Inertia_Axiom.clingo.txt}
\lstinputlisting{Figures/APL_Language/04_Minimal_Knowledge/Check_2_with_Inertia_Axiom.clingo.txt}
...
Most involve reasoning over access control policies (e.g.) and a few utilize ASP as a reasoning tool (e.g.).
However, the work by \citet{liao_extended_2006} is the most relevant for discussion in this thesis.

\subsection{PDC-agent}

\citet{liao_extended_2006} present an extension of the BDI architecture call PDC-agent, which is based off of a policy and contract aware methodology they call BGI-PDC logic\footnotemark.
...
\begin{definition}
    PDC-agent for a single agent is defined as a collection of components grouped into modules.
...
\begin{definition}
    ...
    The role of each component is presented below~\citep{liao_extended_2006}:

    \begin{itemize}
        \item The \textit{Event Treating Engine} is an event hub that passes messages and performs simple message manipulations.
        ...
        \item The \textit{Belief Update} component maintains the agent’s belief database on the basis of events and belief-manipulating actions (such as AddBelief or DeleteBelief).
        \item The \textit{Contract Engine} listens for ExtE events and emits events when relevant contract clauses are satisfied.
        \item The \textit{Policy Engine} evaluates policies.
        \item The \textit{Goal Maintenance} component produces goals.
        \item The \textit{Plan Engine} finds plans to meet goals, according to plans in the Plan Library.
    \end{itemize}
\end{definition}

\begin{definition}
    The \textit{State Module} of a PDC-agent consists of the \textit{Intention Set}, \textit{Goal Set}, and \textit{beliefs}.
    Beliefs constitute an agent’s understanding of its environment, the agent’s internal states, as well as a representation of contracts and policies.
\end{definition}

\begin{definition}
    The \textit{Execution Module} of a PDC-agent consists of the \textit{Execution Engine}, \textit{external actions}, and \textit{internal actions}.
\end{definition}

This architecture, along with the flow of events between its components, is summarized in \cref{fig:pdc_agent_structure}.
An informal representation of the components collective control loop is presented in \cref{fig:pdc_agent_control_loop}.
...
While both the AAA agent architecture and the $\mathcal{AIA}$ agent architecture can allow multiple agents to operate in a shared environment, the AAA and $\mathcal{AIA}$ architectures do not provide any mechanism to coordinate actions between to work towards a common goal.

Similar to the $\mathcal{AIA}$ architecture, PDC-agent distinguishes between external actions and internal actions.
Internal actions can update beliefs, analogous to $\mathcal{TI}$, but they can also emit events to the Event Treating Engine.
...
Instead, knowledge in PDC-agent is formally represented in terms of Domain Conceptualization Language (DCL)~\citep{ji_gao_sasa5_2005} and Concept Instance Pattern (CIP).
For brevity, this thesis will not include the syntax of DCL and CIP as well as a discussion on how these languages are used to formally represent each entity in PDC-agent.
