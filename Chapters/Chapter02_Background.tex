...
\footnotetext{
    ...
    \citet{balduccini_aaa_2008} denote the $obs$ predicate as $obs(FluentName, State, FluentValue)$ whereas \citet{blount_towards_2014} writes $obs(FluentName, FluentValue, State)$ to denote the same history.
...
\begin{definition}
    ...
    and\footnotemark
    \begin{multline}
        F\subseteq\left(\{V(f,s)=y | (f, y) \in \boldsymbol{F} \times \boldsymbol{V}\}\right) \cup (\{permitted(a),
        \neg permitted(a), \\
...
\begin{multline}
    \label{eq:apl_lp_08}
    lp(d: \textbf{normally } permitted(a) \textbf{ if } F) =_{def} \\
        permitted(a) \leftarrow
            lp(F),
            \textbf{not } ab(d),
            \textbf{not } \neg permitted(a).
...
Let $D = \{lp(V(f,s)=y_1) \lor lp(V(f,s)=y_2) \lor \dots \lor lp(V(f,s)=y_n) |  f \in \boldsymbol{F_{lim}}\}$, where $y_1, y_2, \dots, y_n \in \boldsymbol{V}$.
...
\begin{multline}
    ...
    lp(d: \textbf{normally } permitted(a) \textbf{ if } F, I) =_{def} \\
        permitted(a, I) \leftarrow
            lp(F, I),
            \textbf{not } ab(d, I),
            \textbf{not } \neg permitted(a, I).
...
Let $D_0 = \{lp(V(f,s)=y_1, 0) \lor lp(V(f,s)=y_2, 0) \lor \dots \lor lp(V(f,s)=y_n, 0) |  f \in \boldsymbol{F_{lim}}\}$, where $y_1, y_2, \dots, y_n \in \boldsymbol{V}$
...
\footnotetext{
    ...
    Whereas \citet{blount_towards_2014} would write $obs(FluentName, FluentValue, State)$ and $hpd(ActionName, State)$,\citet{gelfond_authorization_2008} write $val(FluentName, FluentValue, State)$ and $occurs(ActionName, State)$, respectively.
}
...
\begin{definition}
    ...
    \begin{gather}
        ...
        prefer(d_i, d_j)
    \end{gather}
    where $s \in S$, $a \in \boldsymbol{A} \cup \{\neg a' | a' \in \boldsymbol{A}\}$, and
    \begin{multline}
        F\subseteq\left(\{V(f,s)=y | (f, y) \in \boldsymbol{F} \times \boldsymbol{V}\}\right) \cup (\{obl(a),
            \neg obl(a), \\
...
\begin{gather}
    ...
    lp(\neg obl(a)) =_{def}
        \neg obl(a). \\
    lp(obl(a) \textbf{ if } F) =_{def}
        obl(a) \leftarrow
            lp(F). \\
    lp(\neg obl(a) \textbf{ if } F) =_{def}
        \neg obl(a) \leftarrow
            lp(F).
\end{gather}
\begin{multline}
    lp(d: \textbf{normally } obl(a) \textbf{ if } F) =_{def}
        obl(a) \leftarrow \\
            lp(F),
            \textbf{not } ab(d),
            \textbf{not } \neg obl(a). \\
...
\lstinputlisting{Figures/AOPL_Language/01_Example/Check_1.clingo.txt}
\lstinputlisting{Figures/AOPL_Language/01_Example/Check_2.clingo.txt}
