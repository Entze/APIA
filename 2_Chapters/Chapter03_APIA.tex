...
To allow for the case when no activities that achieve $F$ are $L$-compliant or for when an agent deliberately chooses act without $L$-compliance, we introduce policy actions $ignore\_not\_permitted(a)$ and $ignore\_neg\_permitted(a)$, where $a$ is a physical action.
...
This scenario can be represented by the following fluents
\begin{equation}
    \dots
\end{equation}
Actions:
\begin{equation}
    \dots
\end{equation}
and the following action description.
\begin{equation}
    \dots
\end{equation}
...
In this first step, Alice observes that she is in Room 1, Bob is in room 4, the door $d_{34}$ is unlocked, and that she has not yet greeted Bob today.
Then, Alice analyzes these observations and interprets unexpected observations using undetected exogenous actions.
None of these observations are unexpected.
...
However, let us transition from this serendipitous example to one where a strongly compliant activity becomes weakly compliant due to an unexpected environmental observation.

In the same scenario, suppose Alice has the following policy:

\begin{equation}
    \dots
\end{equation}

Before the $\mathcal{AIA}$ loop begins, Alice observes that Bob is not busy working.
In the $(paranoid, paranoid)$ mode
