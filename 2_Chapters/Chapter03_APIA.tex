...
\begin{equation}
\begin{split}
\neg permitted(greet(Actor, Person)) if \\
...
normally \neg permitted(move\_through(Actor, Door)) if
...
\begin{equation}
\begin{split}
obl(\neg turn\_lights\_off(Actor, Room)) if
    & person(Person) \\
    & not agent(Person) \\
...
\begin{equation}
\begin{split}
obl(\neg ask\_to\_move\_through(Actor, Door)) if
    lights\_off(ToRoom)
\end{split}
\end{equation}
...
normally obl(\neg enable\_security\_system(Actor)) if
    \neg all\_lights\_off
\end{split}
\end{equation}
